...
Analyzes overlap of invasive/non-invasive BP data. 
...
for col in cols:
    ...
    #Find data from both sources at the same time. 
    id_and_offset = ['patientunitstayid','observationoffset']
    both = inv.merge(noninv, on=id_and_offset, how='inner').dropna()
...
def filter_on_col(col,thresh,pulse):
    ...
    return temp, temp_prop, temp_stays

# How much of the invasive pulse pressure is less than 10? 
pulse10, pulse10prop, pulse10stays = filter_on_col('systemicpulse',10,pulse)

# How often is the non-invasive systolic 20 more than invasive systolic?
sys20,sys20prop, sys20stays = filter_on_col('diff_systolic',-20,pulse)

# How often is the non-invasive mean 20 more than invasive mean?
mean20, mean20prop, mean20stays = filter_on_col('diff_mean',-20,pulse)
...
make_histo(pulse['diff_pulse'],
           ...
           'difference (mmHg)', 'pulse', 'BP Overlap')
# Zoomed in histogram. 
